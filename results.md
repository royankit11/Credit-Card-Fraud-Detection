...
Across all three approaches, the supervised models performed noticeably better than the unsupervised one, but each method showed different strengths and weaknesses. Logistic Regression served as a strong linear baseline, achieving an ROC-AUC of about 0.86 and a PR-AUC around 0.43 when trained on the original features. It correctly classified most non-fraudulent transactions but struggled with the fraud class, missing over a thousand fraudulent cases and producing a relatively low fraud F1-score. When trained on PCA-reduced features, its overall performance dropped further, showing that the dimensionality reduction removed important predictive information for a linear model.

The neural network, FraudNet, consistently outperformed Logistic Regression across nearly every metric. Using the full feature set, it reached an ROC-AUC near 0.90 and a higher PR-AUC of roughly 0.55, reflecting better separation between fraud and non-fraud. It detected more fraudulent cases than Logistic Regression and achieved higher recall and F1-score for the fraud class while only modestly increasing false positives. Like Logistic Regression, the PCA version of FraudNet performed worse, reinforcing the idea that PCA discards useful structure for models that rely on nonlinear relationships. Overall, FraudNet demonstrated that a nonlinear architecture captures more complex fraud patterns than a linear model can.

K-Means, the unsupervised model, performed considerably worse than both supervised methods. After mapping clusters to labels, it achieved about 63% accuracy with fraud precision around 0.61 and recall around 0.73. While the recall seems relatively high, the model generated many false positives, and its PR-AUC was significantly lower, around 0.25–0.30. Internal clustering metrics also showed weak separation, indicating that fraudulent and non-fraudulent transactions do not form clean clusters in feature space. K-Means can still provide useful anomaly signals, but it is not reliable enough to be used as a standalone fraud detector.

Overall, the neural network is the strongest model, providing the best balance between precision and recall and the highest discriminative power. Logistic Regression remains a solid interpretable baseline but struggles with the complexity of fraud patterns, and K-Means should be viewed mainly as an exploratory or auxiliary tool rather than a primary classifier.
